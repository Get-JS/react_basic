...
function checkPassword(passsword) {
  ...
  var eng = passsword.search(/[a-z]/gi);
...
const singupAlertCheck = {
  ...
  password: (password) => {
    if (!password) return { check: true, msg: '필수 값 입니다.' };
    return checkPassword(password);
  },
  passwordConfirm: (password, passwordConfirm) => {
    if (!passwordConfirm) return { check: true, msg: '필수 값 입니다.' };
    else if (password !== passwordConfirm) return { check: true, msg: '비밀번호가 일치하지 않습니다.' };
